...
def scrape_papers(args):
    ...
    output_dir = args.output if args.output else "pdfs"
...
def main():
    ...
    action_group.add_argument('--scrape', '-s', action='store_true', help='Scrape and download papers')
    action_group.add_argument('--check', '-c', action='store_true', help='Check PDF integrity')
    action_group.add_argument('--verify', '-v', action='store_true', help='Verify and deduplicate PDFs')
    action_group.add_argument('--all', '-a', action='store_true', help='Run all operations')

    parser.add_argument('--max', '-m', type=int, default=20, help='Maximum papers to download')
    parser.add_argument('--csv', '-f', type=str, help='CSV file with paper data')
    parser.add_argument('--output', '-o', type=str, help='Output directory for PDFs')
    parser.add_argument('--directory', '-d', type=str, help='PDF directory for check/verify')
